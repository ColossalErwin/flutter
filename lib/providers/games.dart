...
/*
  ...
  */
  List<Game> get trendingGames {
    List<Game> tempGamesNotYetReleased = [];
    List<Game> tempGamesReleased = [];
    for (Game game in _trendingGames) {
      if ((game.releaseDate != null &&
              game.releaseDate!.toDate().isAfter(
                    DateTime.now(),
                  )) ||
          (game.releaseDate == null)) {
        tempGamesNotYetReleased.add(game);
      } else {
        tempGamesReleased.add(game);
      }
    }
    tempGamesNotYetReleased.sort((game1, game2) {
      //80 is given as default value if anticipatedLevel is null
      return ((game2.anticipatedLevel == null) ? 80 : game2.anticipatedLevel as int)
          .compareTo((game1.anticipatedLevel == null) ? 80 : game1.anticipatedLevel as int);
    });
    tempGamesReleased.sort((game1, game2) {
      //80 is given as default value if anticipatedLevel is null
      return ((game2.anticipatedLevel == null) ? 80 : game2.anticipatedLevel as int)
          .compareTo((game1.anticipatedLevel == null) ? 80 : game1.anticipatedLevel as int);
    });
    for (var game in tempGamesReleased) {
      tempGamesNotYetReleased.add(game);
    }
    return tempGamesNotYetReleased;
    //return [..._trendingGames];
  }
          ...
          for (final trashGame in trashGamesList) {
            ...
            if (currentDate.difference(deletedDate) > Duration(days: _timeBeforeEmptyTrash ?? 30)) {
              ...
              if (trashGame['titleImageURL'].startsWith(
                  'https://firebasestorage.googleapis.com/v0/b/my-backlog-1040f.appspot.com/o/users%2F${FirebaseAuth.instance.currentUser!.uid}')) {
                try {
